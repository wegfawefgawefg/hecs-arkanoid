...
use crate::{
    rendering::execute_render_command_buffer,
...
pub fn draw(state: &State, low_res_draw_handle: &mut RaylibTextureMode<RaylibDrawHandle>) {
    match state.game_mode {
        GameMode::Title => {
            title_draw(&state, low_res_draw_handle);
        }
        GameMode::Playing => {
            playing_draw(&state, low_res_draw_handle);
        }
        GameMode::GameOver => {
            game_over_draw(&state, low_res_draw_handle);
        }
    }
}

////////////////////////    PER GAME MODE DRAW FUNCTIONS     ////////////////////////
pub fn title_draw(_state: &State, d: &mut RaylibTextureMode<RaylibDrawHandle>) {
    let mut cursor = Vec2::new(DIMS.x as f32 * 0.28, DIMS.y as f32 * 0.4);
    let title = "HECS-breakout!";
    let size = 20;
...
pub fn playing_draw(state: &State, d: &mut RaylibTextureMode<RaylibDrawHandle>) {
    execute_render_command_buffer(d, &state.render_command_buffer);
}

pub fn game_over_draw(state: &State, d: &mut RaylibTextureMode<RaylibDrawHandle>) {
    let mut cursor = Vec2::new(DIMS.x as f32 * 0.28, DIMS.y as f32 * 0.4);
    let title = "GAME OVER!";
    let size = 20;
    d.draw_text(title, cursor.x as i32, cursor.y as i32, size, Color::WHITE);
